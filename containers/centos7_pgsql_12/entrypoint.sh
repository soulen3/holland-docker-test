...
su -c /usr/pgsql-12/bin/initdb -l postgres 2>>/dev/null >>/dev/null
echo 'host all root 127.0.0.1/32 trust' >> /var/lib/pgsql/12/data/pg_hba.conf 2>>/dev/null >>/dev/null
su -c '/usr/pgsql-12/bin/pg_ctl -D /var/lib/pgsql/12/data start' postgres 2>>/dev/null >>/dev/null
sleep 20
su -c '/usr/pgsql-12/bin/psql -c "CREATE USER root WITH SUPERUSER"' postgres 2>>/dev/null >>/dev/null
